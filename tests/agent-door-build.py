...
"- Your Dojo: $%s a month. Rick builds it, hosts it, and helps you run it; the client's team presses every send. Monthly, flat, any number of seats, cancel any time. A first contact is free."%P['Your Dojo'],
"- Every offer lives on Rick's own stack: nothing for the client to log into, renew or fix. The client's domain, content and data are theirs and leave with them; the build and the hosting end with the subscription.",
